...
The installer runs `stow -d stow -t $HOME <pkg>` from the repo root. Registered packages: `ccstatusline`, `git`, `mise`, `nvim`, `rofi`, `ssh-agent`, `sway`, `systemd`, `tmux`, `zsh`, `k9s`, `nwg-displays`. `claude`, `claude-personal`, `my-scripts`, `pgcli`, `vscode` and `foot` are listed in `STOW_NO_FOLDING` (uses `stow --no-folding`) so the shared target directory does not itself become a symlink.
