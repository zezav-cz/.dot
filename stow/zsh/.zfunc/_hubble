...
_hubble()
{
    ...
    while IFS='\n' read -r comp; do
        ...
        if [ "${comp[1,$endIndex]}" = "$activeHelpMarker" ];then
            __hubble_debug "ActiveHelp found: $comp"
            comp="${comp[$startIndex,-1]}"
            if [ -n "$comp" ]; then
                compadd -x "${comp}"
                __hubble_debug "ActiveHelp will need delimiter"
                hasActiveHelp=1
            fi
    ...
    if [ $hasActiveHelp -eq 1 ]; then
        if [ ${#completions} -ne 0 ] || [ $((directive & shellCompDirectiveNoFileComp)) -eq 0 ]; then
            __hubble_debug "Adding activeHelp delimiter"
            compadd -x "--"
            hasActiveHelp=0
        fi
    fi
    ...
    if [ $((directive & shellCompDirectiveKeepOrder)) -ne 0 ]; then
        __hubble_debug "Activating keep order."
        keepOrder="-V"
    fi
    ...
    if [ $((directive & shellCompDirectiveFilterFileExt)) -ne 0 ]; then
        ...
        __hubble_debug "Calling _describe"
        if eval _describe $keepOrder "completions" completions $flagPrefix $noSpace; then
            __hubble_debug "_describe found some completions"

            # Return the success of having called _describe
            return 0
        else
            __hubble_debug "_describe did not find completions."
            __hubble_debug "Checking if we should do file completion."
